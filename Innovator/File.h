...
std::shared_ptr<Node> eval_file(const std::string & filename)
{
  ...
  env->outer = std::make_shared<Env>(
    ...
    { "subpass", fun_ptr(shared_from_node_list<SubpassDescription, std::shared_ptr<Node>>) },
    ...
    { "renderpass-description", fun_ptr(shared_from_node_list<RenderpassDescription, std::shared_ptr<Node>>) },
