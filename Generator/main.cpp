...
int main(int argc, char* argv[])
{
	...
	if (!std::filesystem::exists(filename)) {
		...
		for (size_t lod = 0; lod < num_lods; lod++) {
			...
			glm::u8vec4* data = new glm::u8vec4[lod_size * lod_size * lod_size];
			for (size_t i = 0; i < lod_size; i++) {
				...
				for (size_t j = 0; j < lod_size; j++) {
					for (size_t k = 0; k < lod_size; k++) {
						...
						data[((i * lod_size) + j) * lod_size + k] = glm::u8vec4(value * 255.0);
					}
				}
			}
			out.write(reinterpret_cast<char*>(data), lod_size * lod_size * lod_size * 4);
		}
	}
